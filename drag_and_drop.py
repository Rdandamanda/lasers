...
def get_mouse_selected(event: Event, screen: Screen) -> list[int]:
    ...
    for element in overlapping:
        print(f"ID: {element}, Type: {screen.tk_canvas.type(element)}")
        print(len(screen.tk_canvas.find_all()))
    
    return overlapping

def on_mouse_grab(event: Event, screen: Screen):
...
def on_mouse_drag(event: Event, screen: Screen):
    global selection_original_coords
